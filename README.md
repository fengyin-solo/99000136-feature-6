...
- **留言发布**：用户可提交留言，选择类型（求助/建议/失物招领），支持多图上传（最多 6 张）
- **多图排序**：支持拖拽 / 按钮调整图片顺序，列表、详情、后台审核处顺序保持一致
- **草稿暂存**：填写内容与图片自动保存到本地，离开页面再回来可恢复，可导出 / 导入草稿文件
- **提交幂等**：同一条草稿重复提交只会生成一条待审核留言
...
2. 访问安装脚本创建数据库：
   ...
   ```
   已安装过旧版本（单图）的站点，请额外执行多图升级迁移：
   ```
   database/migration_add_message_images.sql
   ```
